...
#reproject coordinates for Urbana
urbana.locations.lonlat = cbind(urbana_full$long, urbana_full$lat)
urbana.locations.meters = project(urbana.locations.lonlat, proj = "+init=epsg:26971")
urbana_meters = as.data.frame(cbind(urbana.locations.meters[,1], urbana.locations.meters[,2]))
names(urbana_meters) = c("x", "y")
urbana_full = cbind(urbana_full, urbana_meters)
...
urbana_full14 <- cbind(urbana_full14[,1], urbana_full14[4:5])
names(urbana_full14)[1] <- "response"
...
# fit GLM
glm.fit = glm(response ~ . -x -y, data = train14, family = binomial)
summary(glm.fit)
